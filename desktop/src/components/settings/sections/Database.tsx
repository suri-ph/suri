...
export function Database({ systemData, groups, isLoading, onClearDatabase, onGroupsChanged }: DatabaseProps) {
  ...
  const startEditing = (member: AttendanceMember, field: 'name' | 'email' | 'role') => {
  ...
  const startEditingGroup = (group: AttendanceGroup, field: 'name' | 'description') => {
    setEditingGroup({ groupId: group.id, field });
    setEditValue(group[field] || '');
  };

  const saveEdit = async (personId: string, field: 'name' | 'email' | 'role') => {
    if (editValue.trim() === '') {
      if (field === 'name') {
        ...
        setGroupsWithMembers(prev =>
          prev.map(group => ({
            ...
            members: group.members.map(m =>
              m.person_id === personId ? { ...m, [field]: editValue.trim() || undefined } : m
            )
          }))
        );
        cancelEditing();
  ...
  const saveGroupEdit = async (groupId: string, field: 'name' | 'description') => {
    ...
    try {
      ...
      if (success) {
        ...
        setGroupsWithMembers(prev =>
          prev.map(group =>
            group.id === groupId ? { ...group, [field]: editValue.trim() || undefined } : group
          )
        );
  ...
  const handleKeyDown = (e: React.KeyboardEvent, personId: string, field: 'name' | 'email' | 'role') => {
    if (e.key === 'Enter') {
  ...
  const handleGroupKeyDown = (e: React.KeyboardEvent, groupId: string, field: 'name' | 'description') => {
    if (e.key === 'Enter') {
  ...
  const totalMembers = groupsWithMembers.reduce((sum, group) => sum + group.members.length, 0);
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg p-4 bg-white/5 border border-white/10">
          ...
          <div className="text-2xl font-semibold text-white">{groups.length}</div>
        </div>
        <div className="rounded-lg p-4 bg-white/5 border border-white/10">
        ...
        <div className="rounded-lg p-4 bg-white/5 border border-white/10">
          ...
          <div className="text-2xl font-semibold text-white">{systemData.totalPersons}</div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {filteredData.length === 0 ? (
          ...
          filteredData.map(group => {
            ...
            return (
              <div key={group.id} className="rounded-lg border border-white/10 bg-white/5 overflow-hidden">
                {/* Group Header */}
                <div className="w-full px-4 py-3 flex items-center justify-between hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    >
                      <i className={`fa-solid fa-chevron-${isExpanded ? 'down' : 'right'} text-white/40 text-xs transition-transform`}></i>
                    </button>
                    <div className="flex-1 min-w-0 text-left space-y-1">
                      ...
                      {editingGroup?.groupId === group.id && editingGroup.field === 'name' ? (
                        <input
                          ...
                          onKeyDown={(e) => handleGroupKeyDown(e, group.id, 'name')}
                          autoFocus
                        ...
                        >
                          {group.name}
                          {savingGroup === group.id && <span className="ml-2 text-white/40"><i className="fa-solid fa-spinner fa-spin"></i></span>}
                        </div>
                      )}
                      {/* Group Description */}
                      {editingGroup?.groupId === group.id && editingGroup.field === 'description' ? (
                        <input
                          ...
                          onKeyDown={(e) => handleGroupKeyDown(e, group.id, 'description')}
                          autoFocus
                          disabled={savingGroup === group.id}
                        ...
                        <div
                          onClick={() => startEditingGroup(group, 'description')}
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0 space-y-2">
                                {/* Name */}
                                <div>
                                  {editingMember?.personId === member.person_id && editingMember.field === 'name' ? (
                                    <input
                                      type="text"
                                      value={editValue}
                                      onChange={(e) => setEditValue(e.target.value)}
                                      onBlur={() => saveEdit(member.person_id, 'name')}
                                      onKeyDown={(e) => handleKeyDown(e, member.person_id, 'name')}
                                      autoFocus
                                    ...
                                    <div
                                      onClick={() => startEditing(member, 'name')}
                                ...
                                {/* Role */}
                                <div>
                                  {editingMember?.personId === member.person_id && editingMember.field === 'role' ? (
                                    <input
                                      type="text"
                                      value={editValue}
                                      onChange={(e) => setEditValue(e.target.value)}
                                      onBlur={() => saveEdit(member.person_id, 'role')}
                                      onKeyDown={(e) => handleKeyDown(e, member.person_id, 'role')}
                                      autoFocus
                                      disabled={savingMember === member.person_id}
                                    ...
                                    <div
                                      onClick={() => startEditing(member, 'role')}
                                ...
                                {/* Email */}
                                <div>
                                  {editingMember?.personId === member.person_id && editingMember.field === 'email' ? (
                                    <input
                                      type="email"
                                      value={editValue}
                                      onChange={(e) => setEditValue(e.target.value)}
                                      onBlur={() => saveEdit(member.person_id, 'email')}
                                      onKeyDown={(e) => handleKeyDown(e, member.person_id, 'email')}
                                      autoFocus
                                      disabled={savingMember === member.person_id}
                                    ...
                                    <div
                                      onClick={() => startEditing(member, 'email')}
                              ...
                              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleDeleteMember(member.person_id, member.name);
                                    }}
